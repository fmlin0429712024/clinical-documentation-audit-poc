...
- **PR #5 (the fix above) hit the same workflow-validation skip as PR #3**: it also edits `.github/workflows/ci.yml`, so the App's own guard (workflow file must match `main`) skipped the actual review again — confirmed via the job log (`gh run view --job=91889281121 --log`), same `Action skipped due to workflow validation error` message as PR #3. Expected, not a new bug; noted here explicitly since a later PR's own `claude-review` comment flagged this exact gap in this section as under-documented.
