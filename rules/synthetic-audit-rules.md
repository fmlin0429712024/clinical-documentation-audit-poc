...
| SYN-ICHD-02 | non-deterministic | `treatment_note` documents a patient refusing or discontinuing treatment early | `treatment_note`, `follow_up_note`, and the narrative use-case description in the `treatment-refusal-review` skill | Apply the skill's use-case description; cite which of the four judgment points are documented vs. an evidence gap | Confirm adequate, request clarification, or flag the gap |
